    ...
    class Program     {
        ...
        static void Main(string[] args)
        {
            ...
            while (true){
                ...
                } else if (c == '*'){		//当前输入字符是 *，说明已读出乘号或遇到乘方算符的首字母
                    preCh = c;					//暂存已读的 *
                    c = getChar(sr);//读取新字符

                    if (c == preCh){		//多读的字符也是 *，说明已读出了乘方算符
                        token = new Token(Token_Type.POWER, "**");

                    } else {				//多读的字符不是 *，说明仅仅是乘号
                        ungetChar(sr);	//将多读的字符退回输入流
                        token = new Token(Token_Type.MUL, "*");
                    }

                } else if (char.IsLetter(c)){	//当前输入字符是字母，说明可能遇到了标识符的首字母
                    string buf = ""; //初始化临时串
                ...
                }else if (char.IsDigit(c)){	//当前输入字符是数字，说明遇到了数值常量的首个数字字母
                    string buf = ""; //初始化临时串
                    buf = buf + c;//首数字存入临时串
